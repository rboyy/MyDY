// Auto-generated signing module based on dy_ab.js bdms SDK
// Uses direct signing function (bdms.init._v[2].p[42]) instead of XHR interception
// Loaded by ABogusSigner.kt AFTER browser shim
...
// initABogus: no-op for compatibility (bdms SDK auto-initializes)
function initABogus() {
    return typeof window.bdms !== "undefined" && typeof window.bdms.init === "function" && typeof window.bdms.init._v !== "undefined";
}

// getABogus: wrapper matching the original abogus.js interface
function getABogus(query, data, uri) {
    try {
        var result = get_ab(query, data || "");
        if (result && result.length > 20) {
            return JSON.stringify({ ok: true, value: result });
        }
        return JSON.stringify({ ok: false, error: "get_ab returned invalid result" });
    } catch(e) {
